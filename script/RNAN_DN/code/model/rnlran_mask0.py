...
class _ResGroup_Mask0(nn.Module):
    def __init__(self, conv, n_feats, kernel_size, act, res_scale):
        super(_ResGroup_Mask0, self).__init__()
        modules_body = []
        #modules_body.append(common.NonLocalBlock2D(n_feats, n_feats // 2))
        modules_body.append(common.ResGroup_Mask0(conv, n_feats, kernel_size, bias=True, bn=False, act=nn.ReLU(True), res_scale=1))
        # if we don't use group residual, donot remove the following conv
        modules_body.append(conv(n_feats, n_feats, kernel_size))
        self.body = nn.Sequential(*modules_body)
...
class RNLRAN_Mask0(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        super(RNLRAN_Mask0, self).__init__()
